...
//                               sh 'helm repo update'
                              sh 'helm install --wait stable/distributed-jmeter --name distributed-jmeter-${JOBNAME}-${BUILD_NUMBER} --set server.replicaCount=${noOfSlaveNodes},master.replicaCount=0,image.repository=gvasanka/tqe-jmeter-slave,image.tag=5.1.1,image.pullPolicy=Always'
